...
Match-level train/test splits:  an entire match is either training data or test data, never both. The model is judged only on matches it has never seen.
Model choice : a calibrated Logistic Regression was preferred over a flashier tree ensemble because its probabilities are trustworthy and its behaviour is interpretable.
Reported numbers are the post-leakage, honest ones (AUC 0.865), not the inflated 99%.


**How it works**

Data — raw Cricsheet ball-by-ball JSON for IPL matches.
Feature engineering — from each delivery in the second innings, derive the match state: runs required, balls remaining, wickets in hand, current & required run rate, target, etc.
Label — did the chasing team ultimately win? (1/0)
Model — Logistic Regression with probability calibration, validated with match-level splits.
App — a Streamlit interface where you enter a live match state and get an instant win-probability read-out.
